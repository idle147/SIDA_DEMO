...
from models.llava.constants import DEFAULT_IMAGE_TOKEN, IGNORE_INDEX, IMAGE_TOKEN_INDEX
...
from .magic_brush_dataset import MagicBrushDataset
...
def collate_fn(batch, tokenizer=None, conv_type="llava_v1", use_mm_start_end=True, local_rank=-1):
    ...
    """
        attention_masks用于指示哪些输入标记应该被注意（即参与计算注意力），哪些应该被忽略。通常，它与填充标记（padding token）一起使用。
        attention_mask 标记了哪些是有效的输入（即非填充标记），哪些是填充标记
    """
    ...
    for conversation, target in zip(conversation_list, targets):
        ...
        # 除了IMAGE和Instruction文本外, 其他所有的内容都无视
...
class HybridDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for dataset in self.datasets:
            ...
            elif dataset == "magic_brush":
                self.all_datasets.append(
                    MagicBrushDataset(base_image_dir, tokenizer, vision_tower, samples_per_epoch, precision, image_size)
                )
    ...
    def __init__(
        ...
        if len(splits) == 2:
            ...
            for item in loaded_images:
                item = item.copy()
                if ds == "refclef":
                    item["file_name"] = os.path.join(base_image_dir, "images/saiapr_tc-12", item["file_name"])
