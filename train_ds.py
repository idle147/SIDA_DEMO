...
import torch.autograd as autograd  # 导入异常检测工具
...
def main(args):
    ...
    # 加入[SEG]
    num_added_tokens = tokenizer.add_tokens(["[SEG]", "[DET]"])
    ...
    # Initialize model
    model: LISAForCausalLM = LISAForCausalLM.from_pretrained(args.version, torch_dtype=torch_dtype, **model_args)
    ...
    # Initialize vision modules and freeze certain parameters
    ...
    if lora_r > 0:
        """
        从模型中查找符合条件的线性层（torch.nn.Linear），并且这些线性层的名字中不能包含特定的字符串
        （如 "visual_model", "vision_tower", "mm_projector", "text_hidden_fcs"）。
        然后，筛选出名字中包含lora_target_modules中指定的字符串的线性层。
        返回符合条件的线性层的名字（lora_module_names）。
        """
    ...
    # make text_hidden_fcs, mask_decoder, lm_head, embed_token, attention_module, fc_1, fc_detection
    ...
    # 手动初始化自定义层的权重
    if args.local_rank == 0:
        model_engine.module.initialize_weights()
        print("Custom weights initialized.")
...
def validate(val_loader, model_engine, epoch, writer, args):
    ...
    for input_dict in tqdm.tqdm(val_loader):
        ...
        # 强制将所有输入张量转换为 float32 并移动到正确的设备
        input_dict["images"] = input_dict["images"].float().to(device)
        input_dict["images_clip"] = input_dict["images_clip"].float().to(device)
